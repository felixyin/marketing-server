...
client.on('connect', function () {
    client.subscribe('marketing-search', function (err) {
        if (!err) console.log('mqtt订阅的频道：' + 'marketing-search')
    })
...
client.on('message', function (topic, message) {
    ...
    try {
        ...
        switch (topic) {
            case 'marketing-search': // 通过搜索引擎爬取站点url
